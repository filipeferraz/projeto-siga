...
public class Sincronizador {
	...
	private static final int MAX_ATERACOES = 50; // n�mero m�ximo de altera��es
	// em um �nico passo
	...
	// Executa algoritmo de compara��o entre as listas xml e tabela e
	// preenche
	...
	private void encaixar() throws AplicacaoException {
		...
		while (oAntigo != null || oNovo != null) {
			if ((oAntigo == null)
				...
				// incluido
				list.add(new Item(Operacao.incluir, oNovo, null));
				if (iNovo.hasNext())
				...
				list.add(new Item(Operacao.excluir, null, oAntigo));
				// excluir.add(oTabela);
				if (iAntigo.hasNext())
					oAntigo = iAntigo.next();
				else
				...
				if (!oNovo.semelhante(oAntigo, 0))
					list.add(new Item(Operacao.alterar, oNovo, oAntigo));
			...
			if (list.size() > MAX_ATERACOES) {
				...
				// "N�mero m�ximo de altera��es em um �nico passo ultrapassado! ("
				// + MAX_ATERACOES + ")");
			}
		}
		iNovo = null;
